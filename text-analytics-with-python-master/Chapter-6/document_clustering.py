...
def plot_hierarchical_clusters(linkage_matrix, movie_data, figure_size=(8,12)):
    ...
    print 'Llegó aquí'.decode('utf-8')
    movie_titles = movie_data['Title'].values.tolist()
    # plot dendrogram
    imprime('también aquí.')
    ...
    plt.tick_params(axis= 'x',   
                    ...
                    top='off',
                    labelbottom='off')
    imprime('Pasó tick_params')
    plt.tight_layout()
    imprime('ejecutó plt.tight_layout')
    plt.savefig('ward_hierachical_clusters.png', dpi=None)
    imprime('terminó la función')
    dendrograma = Image.open("ward_hierachical_clusters.png")
    dendrograma.show()

# build ward's linkage matrix    
linkage_matrix = ward_hierarchical_clustering(feature_matrix)
# plot the dendrogram
imprime('Imprimiría la plot herarchichal_clusters')
